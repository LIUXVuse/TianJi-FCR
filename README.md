...
- **距離斷頭指標**：融資維持率與斷頭距離、合約爆倉距離 🆕
- **資產配置圖**：攻守分明的圓餅圖，支援細項展開 🆕
- **拖放排序**：儀表板組件自由排列，刷新記憶 🆕
- **台股即時報價**：串接證交所 API
- **幣安即時價格**：一鍵更新所有加密貨幣現價
- **AI 財務軍師**：結合八字命理的專業財務建議
- **八字自動計算**：輸入生辰自動算出四柱八字
- **Supabase 雲端同步**：支援跨裝置資料漫遊
- **歷史淨值追蹤**：自動/手動快照淨值，並繪製趨勢圖
- **量化回測系統**：整合台股量化分析工具
...
- ✅ **現貨 (Spot)**：無槓桿持倉
- ✅ **合約 (Future)**：支援槓桿
- ✅ **強平價輸入**：可手動輸入 Cross Margin 強平價 🆕
- ✅ **爆倉距離 %**：顯示距離強平的百分比，低於 10% 變紅閃爭 🆕
- ✅ **幣安即時價格**：自動取得幣價
- ✅ **一鍵更新**：批次更新所有持倉現價
- ✅ 自動匯率換算 (USDT → TWD)
...
- ✅ **圓餅圖**：台股 / 美股 / 幣圈 / 現金 佔比
- ✅ **細分展開**：點擊顯示個股、幣種明細
- ✅ **色彩分類**：融資/合約(紅橙)、現貨/現金(藍綠)

### 6. 拖放排序 🆕

- ✅ **儀表板組件自由排序**：拖放調整位置
- ✅ **鎖定/解鎖模式**：預設鎖定避免誤觸
- ✅ **localStorage 記憶**：刷新頁面保持排序
...
│   ├── CryptoSection.tsx    # 加密貨幣模組 (含強平價輸入)
│   ├── DebtSection.tsx      # 負債模組
│   ├── AllocationChart.tsx  # 資產配置圓餅圖 🆕
│   ├── DraggableDashboard.tsx # 拖放排序容器 🆕
...
- ✨ **全新視覺設計**：賽博龐克風格大改版
- ✨ **玻璃擬態 (Glassmorphism)**：半透明卡片 + 模糊背景
- ✨ **漸層色彩系統**：青色/翠綠/紫色/金色漸層
- ✨ **科幻字體 (Orbitron)**：數字顯示更具未來感
- ✨ **動態進度條**：目標達成進度填充動畫
- ✨ **Logo 光暈效果**：導航列 Logo 脈動發光
- ✨ **卡片懸浮效果**：Hover 時上浮陰影變化
- ✨ **按鈕按壓回饋**：點擊時微縮動畫
- 🔧 優化資產膨脹預測區塊 UI
- 🔧 優化波段分析顯示效果
- 🔧 優化目標追蹤進度條動畫
...
- ✨ **距離斷頭指標**：台股融資顯示維持率 + 斷頭距離
- ✨ **爆倉距離指標**：幣圈合約顯示距離強平百分比
- ✨ **強平價輸入**：Cross Margin 可手動輸入強平價
- ✨ **資產配置圓餅圖**：攻守分明色彩，支援細項展開
- ✨ **拖放排序**：儀表板組件自由排列，刷新記憶
- 🔧 修復上櫃股票 (OTC) 報價問題
- 🔧 修復全局刷新幣圈 UI 不同步問題
